...
@Route("counter")
class Page : Div(), ComposablePage, BeforeEnterObserver {
...
2. Each Element creates and configures Vaadin Component instances. Some of the Elements do nothing here and just
   proparate Component from nested Elements.
...
Components according to widget's properties.

The other widgets are generally interited from `StatelessWidget` and `StatefulWidget` classes. It means that their
elements just propagate already rendered Vaadin Component from nested widgets elements.
...
state: they don’t have any properties that change over time. These widgets subclass `StatelessWidget`. However, if the
widget properties need to change based on user interaction or other factors, the `StatefulWidget`
...
2. Attach new widgets to already created Elements if possible. The old Element is reused only if the previous Widget Key
   ...
   * The obsolete Element are removed from cache if there is no custom logic in Element side. For example,
...
a childs and their order could be changed during refresh. Without specified key it will be to hard for Framework to
determine what StatefulElement should be used for new rebuilded widget because the default key doesn't store any unique
identifiers.

Here we set a key for nested `TodoItemWidget` childs because their order could be changer later:
